...
import {COLORS, colorClasses, colorMenuClasses} from '../Data/app';
export function Color({ onColorClick, onBackColorClick,onChange,value,speak, dispatch,state}){
	const colors = COLORS;
//Speak Colors on Homescreen when clicked
	const handleSpeech = (color)=>{
		if(speak) speak(color,dispatch,state);
	}
		...
		<div className={`flex flex-col  lg:place-items-center md:place-items-center sm:place-items-center place-items-center m-0 p-0 lg:w-screen md:w-screen sm:screen max-w-screen `}>
				...
				<div className='flex md:text-6xl text-3xl font-bold m-10 ' style={{ fontFamily: '"DynaPuff", system-ui' }}>
					...
					<ul className="flex flex-row place-items-center h-screen text-xl ">
						{colors.map((color,i) => (
						<li key={i} className={`${colorMenuClasses[color.value]} ${colorClasses[color.value]} w-full h-full animate-pulse cursor-pointer `}onClick={() => handleSpeech(color.value)} >
						</li>
						))}	
					</ul>
				</div>
